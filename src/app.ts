...
const BASE_URL = 'https://listado.mercadolibre.com.mx/celulares#D[A:celulares]';
const MAX_RETRIES = 3; // Número máximo de reintentos
const MAX_PRODUCTS = 10; // Limitar el scraping a 10 productos
...
async function scrapeProduct(url: string) {
  ...
  const product = await page.evaluate(() => {
    ...
    return { title, price, url: window.location.href, imageUrl };
  });
